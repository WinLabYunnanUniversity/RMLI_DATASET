...
file_dir = 'F:/dataset/RML/RML2016.10a_dict.pkl'
...
model.load_state_dict(torch.load('./data/saved_Convnetmodel(rml2016a).pth', map_location=device))
model.eval()
...
for snr in all_data[3]:
    ...
    for i in range(len(all_data[-1])):
        ...
        for pnr in range(-10, 12, 2):
            v_am = get_distubution_power(n_power, pnr) ** 0.5
            v_vector = v * v_am
            data_v = np.squeeze(data_mod+v_vector)
            dict[(mod,snr,pnr)]=data_v

def save_obj(obj, name):
    with open(name + '.pkl', 'wb') as f:
        pickle.dump(obj, f, pickle.HIGHEST_PROTOCOL)
savepath ='F:\dataset\RML\RML2016.10a_int'
